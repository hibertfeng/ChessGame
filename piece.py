...
class Piece:
    ...
    # Name of the pieces
    nomPiece=(VIDE,'KING','QUEEN','ROOK','KNIGHT','BISHOP','PAWN')
    ...
    deplacements_ROOK=(-10,10,-1,1)
    deplacements_BISHOP=(-11,-9,11,9)
    deplacements_KNIGHT=(-12,-21,-19,-8,12,21,19,8)
    ...
    def __init__(self,nom=VIDE,couleur=''):
        ...
        - 'nom' as name (KING, QUEEN...);
        - 'couleur' as color (black,white);
        - 'valeur' as its value"""
    ...
    def pos2_KING(self,pos1,cAd,echiquier,dontCallIsAttacked=False):

        """Returns the list of moves for the king :
        - which is at square 'pos1'
        - which opponent color is 'cAd' (black,white)
        ...
        for i in (self.deplacements_ROOK+self.deplacements_BISHOP):
        ...
        if(c=='black'):
            if(echiquier.white_can_castle_63):
                ...
                if(echiquier.cases[63].nom=='ROOK' and \
                echiquier.cases[63].couleur=='black' and \
                echiquier.cases[61].isEmpty() and \
                echiquier.cases[62].isEmpty() and \
                echiquier.is_attacked(61,'white')==False and \
                echiquier.is_attacked(62,'white')==False and \
                echiquier.is_attacked(pos1,'white')==False):
                    liste.append((pos1,62,''))
            if(echiquier.white_can_castle_56):
                # S'il y a une ROOK en 56, etc...
                if(echiquier.cases[56].nom=='ROOK' and \
                echiquier.cases[56].couleur=='black' and \
            ...
            if(echiquier.black_can_castle_7):
                if(echiquier.cases[7].nom=='ROOK' and \
                echiquier.cases[7].couleur=='white' and \
            ...
            if(echiquier.black_can_castle_0):
                if(echiquier.cases[0].nom=='ROOK' and \
                echiquier.cases[0].couleur=='white' and \
    ...
    def pos2_ROOK(self,pos1,cAd,echiquier):
        
        """Returns the list of moves for a ROOK :
        - at square number 'pos1' (0 to 63)
        - opponent color is cAd (black,white)"""
        
        liste=[]
        
        for k in self.deplacements_ROOK:        
            j=1
            while(True):
    ...
    def pos2_KNIGHT(self,pos1,cAd,echiquier):
        ...
        for i in self.deplacements_KNIGHT:
            n=self.tab120[self.tab64[pos1]+i]
            if(n!=-1):
                if(echiquier.cases[n].isEmpty() or echiquier.cases[n].couleur==cAd):
                    liste.append((pos1,n,''))
    ...
    def pos2_BISHOP(self,pos1,cAd,echiquier):
        ...
        for k in self.deplacements_BISHOP:
            j=1
            while(True):
                n=self.tab120[self.tab64[pos1] + (k * j)]
                if(n!=-1): # as we are not out of the board
    ...
    def pos2_PAWN(self,pos1,couleur,echiquier):
        ...
        if(couleur=='black'):
            ...
            if(n!=-1):
                if(echiquier.cases[n].couleur=='white' or echiquier.ep==n):
        ...
        # Black PAWN ---------------------------------------------------    
            ...
            n=self.tab120[self.tab64[pos1]+9]
            if(n!=-1):
                if(echiquier.cases[n].couleur=='black' or echiquier.ep==n):
                    if(n>55):
                        liste.append((pos1,n,'q'))
                        liste.append((pos1,n,'r'))
                        liste.append((pos1,n,'n'))
